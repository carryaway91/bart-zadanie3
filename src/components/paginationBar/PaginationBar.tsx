...
const PaginationBar: React.FC<IProps> = ({
  ...
  const paginateNum =
    paginatedArray &&
    paginatedArray.map((array: any, idx: number) => {
      return (
        <PageSelector
          key={idx}
          selected={page === idx}
  ...
  return (
    <Container>
      {page >= 5 ? (
        <button onClick={() => handlePrevFive()}>
          <img src={Left} alt="left arrows" width="20" />
        </button>
      ) : (
        <Fill />
      )}
      <Numbers>{paginateNum.slice(from, to)}</Numbers>
      {to < paginatedArray.length ? (
        <button onClick={() => handleNextFive()}>
